...
main project abspath: %s""" % (name, uri, source_dir, target_dir_rp, source_dir_rp))
                        ...
                        if value == optobject['value']:
                            for name, depobject in optobject['depends'].items():
                                add_module(current_module, name,
                                           ...
                                           depobject)
        Subproject.set_local_ignores(root, subprojects_dir, modules)
        return root, modules
    ...
    @staticmethod
    def set_local_ignores(root, subprojects_dir, modules):
        vcs_class(root.directory).set_local_ignores(root, subprojects_dir, modules)
...
Please either remove the local clone, or fix its remote.""" % (self.directory, current_origin, self.url.geturl()))
    ...
    def set_local_ignores(root, subprojects_dir, modules):
        ...
        exclude_path = os.path.join(root.directory, ".git", "info", "exclude")
        exclude_quark = []
        if modules:
